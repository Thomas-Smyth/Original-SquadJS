...
export default class SquadServer {
  ...
  async getServerInfo() {
    try {
      ...
      let {
        currentLayer,
        nextLayer
      } = await this.rcon.getCurrentAndNextLayer();
      ...
      console.log(`${this.id} throw an error when fetching server information`);
      this.layerChange = false;
      this.nextLayerChange = false;
    }
  }
  ...
  async setServerInfo() {
    try {
      ...
      console.log(`${this.id} throw an error when updating server information`);
    }
  }
